...
(define-constant ERR-ESCROW-NOT-FOUND (err u423))
(define-constant ERR-ESCROW-LOCKED (err u424))
(define-constant ERR-TRUST-FUND-LOCKED (err u425))
(define-constant ERR-TRUST-FUND-EXISTS (err u426))
(define-constant ERR-TRUST-FUND-NOT-FOUND (err u427))
...
;; Feature 7: Spending Analytics
(define-map spending-by-category
  {
    parent: principal,
    child: principal,
...
;; Feature 9: Escrow for Large Purchases
...
(define-map escrows
  { id: uint }
  {
    ...
    child: principal,
    amount: uint,
    status: (string-ascii 12), ;; "REQUESTED", "LOCKED", "RELEASED"
    description: (string-ascii 64)
  }
)

;; Feature 10: Time-Locked Trust Fund
(define-map trust-fund
  { beneficiary: principal }
  { release-height: uint, balance: uint }
)
...
(define-read-only (get-achievement-meta (id uint))
  (map-get? achievement-meta { id: id })
)
...
(define-public (request-loan (parent principal) (amount uint))
  (let (
    (child tx-sender)
...
(define-public (repay-loan (loan-id uint) (amount uint))
  (let (
    ...
    (let (
      (new-balance (- (get balance loan) amount))
    )
      (if (is-eq new-balance u0)
        (ok (map-set loans { id: loan-id } (merge loan { balance: u0, status: "PAID" })))
...
;; --------------------------------------------------------------------------
;; Feature 9: Escrow for Large Purchases Implementation
;; --------------------------------------------------------------------------

(define-public (request-escrow (parent principal) (amount uint) (description (string-ascii 64)))
  (let (
    (child tx-sender)
...
(define-public (approve-escrow (id uint))
  (let (
    (parent tx-sender)
    (escrow (unwrap! (map-get? escrows { id: id }) ERR-ESCROW-NOT-FOUND))
    (current-vault (default-to u0 (get balance (map-get? vault-balances { parent: parent }))))
  )
    ;; Assertions
    (asserts! (is-eq (get parent escrow) parent) ERR-UNAUTHORIZED)
    (asserts! (is-eq (get status escrow) "REQUESTED") ERR-BOUNTY-WRONG-STATE)
    (asserts! (>= current-vault (get amount escrow)) ERR-INSUFFICIENT-ALLOWANCE)

    ;; 1. Lock funds (Deduct from vault)
    (map-set vault-balances { parent: parent } { balance: (- current-vault (get amount escrow)) })

    ;; 2. Update Escrow status
    (ok (map-set escrows { id: id } (merge escrow { status: "LOCKED" })))
  )
)
...
(define-public (release-escrow (id uint))
  (let (
    ...
    ;; Only parent can release
    (asserts! (is-eq (get parent escrow) parent) ERR-UNAUTHORIZED)
    (asserts! (is-eq (get status escrow) "LOCKED") ERR-ESCROW-LOCKED)

    ;; 1. Transfer to Child
    (try! (as-contract (stx-transfer? (get amount escrow) tx-sender (get child escrow))))

    ;; 2. Mark as released
    (ok (map-set escrows { id: id } (merge escrow { status: "RELEASED" })))
  )
)

(define-public (revoke-escrow (id uint))
  (let (
    (parent tx-sender)
    (escrow (unwrap! (map-get? escrows { id: id }) ERR-ESCROW-NOT-FOUND))
    (current-vault (default-to u0 (get balance (map-get? vault-balances { parent: parent }))))
    ...
    (if (is-eq (get status escrow) "LOCKED")
      (begin
        ;; Refund to vault
        (map-set vault-balances { parent: parent } { balance: (+ current-vault (get amount escrow)) })
        ;; Delete
        (ok (map-delete escrows { id: id }))
      )
      (begin
        ;; Just delete if only REQUESTED
        (asserts! (is-eq (get status escrow) "REQUESTED") ERR-BOUNTY-WRONG-STATE)
        (ok (map-delete escrows { id: id }))
      )
    )
...
;; --------------------------------------------------------------------------
;; Feature 10: Time-Locked Trust Fund Implementation
;; --------------------------------------------------------------------------

(define-public (create-trust-fund (beneficiary principal) (release-height uint) (amount uint))
  (let (
    (parent tx-sender)
    (parent-balance (default-to u0 (get balance (map-get? vault-balances { parent: parent }))))
  )
    ;; 1. Validation
    (asserts! (is-none (map-get? trust-fund { beneficiary: beneficiary })) ERR-TRUST-FUND-EXISTS)
    (asserts! (> release-height stacks-block-height) ERR-INVALID-INTERVAL)
    (asserts! (>= parent-balance amount) ERR-INSUFFICIENT-ALLOWANCE)
    ...
    ;; 3. Create Fund
    (ok (map-set trust-fund { beneficiary: beneficiary }
      { release-height: release-height, balance: amount }
    ))
  )
)

(define-public (add-to-trust-fund (beneficiary principal) (amount uint))
   (let (
    (parent tx-sender)
    (fund (unwrap! (map-get? trust-fund { beneficiary: beneficiary }) ERR-TRUST-FUND-NOT-FOUND))
    (parent-balance (default-to u0 (get balance (map-get? vault-balances { parent: parent }))))
  )
    (asserts! (>= parent-balance amount) ERR-INSUFFICIENT-ALLOWANCE)
...
(define-public (claim-trust-fund)
  (let (
    (beneficiary tx-sender)
    (fund (unwrap! (map-get? trust-fund { beneficiary: beneficiary }) ERR-TRUST-FUND-NOT-FOUND))
  )
    ;; 1. Check Time Lock
    (asserts! (>= stacks-block-height (get release-height fund)) ERR-TRUST-FUND-LOCKED)
    
    ;; 2. Transfer
    (try! (as-contract (stx-transfer? (get balance fund) tx-sender beneficiary)))
    
    ;; 3. Close Fund
    (ok (map-delete trust-fund { beneficiary: beneficiary }))
  )
)

(define-read-only (get-trust-fund (beneficiary principal))
  (map-get? trust-fund { beneficiary: beneficiary })
)
